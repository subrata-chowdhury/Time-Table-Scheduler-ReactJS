...
const WorkingHourBarChat = ({ dayNames = ["Tue", "Wed", "Thu", "Fri", "Sat"], perDayValue = [5, 0, 8, 1, 2], maxValue = 8 }) => {
    let bars = [];
    if (dayNames.length !== perDayValue.length)
        ...
        for (let index = 0; index < perDayValue.length; index++) {
            bars.push(<div className="bar-container" key={index}>
                    <Bar value={perDayValue[index]} maxValue={maxValue}/>
                    <div className="bar-title">{dayNames[index]}</div>
                </div>);
        }
    }
    return (<div className="working-hour-barchat-container-wrapper">
...
const Bar = ({ value = 0, maxValue = 8 }) => {
    return (<div className="bar" title={value.toString()}>
